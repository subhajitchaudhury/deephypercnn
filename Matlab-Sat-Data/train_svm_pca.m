...
load('indian_pines_data_pca.mat');

% [comp,recon]=pcExtract(permute(X,[2,1,3,4]),50);
% 
% X_r=permute(comp,[2,1,3,4]);

[num_pix,num_ch,h,w]=size(X_r);

train_num=round(0.8*num_pix);
test_num=num_pix-train_num;

train_X=reshape(X_r(1:train_num,:,:,:),[train_num,h*w*num_ch]);
train_y=labels(1:train_num);

test_X=reshape(X_r(train_num+1:end,:,:,:),[test_num,h*w*num_ch]);
...
save('./results/svm_linear_pca.mat','model','predict_label', 'accuracy', 'prob_values');
...
[predict_label, accuracy, prob_values] = svmpredict...
    (double(test_y), double(test_X), model);
save('./results/svm_rbf_pca.mat','model','predict_label', 'accuracy', 'prob_values');
